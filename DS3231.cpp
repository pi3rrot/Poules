...
{
    static char output[] = "xx-xx:xx:xx-xxxxxxx";
    ...
    if (hour<10)
        ...
        output[3]=char((hour / 10)+48);
    output[4]=char((hour % 10)+48);
 
 
    if (min<10)
        output[6]=48; // "0" en ASCII
    else
        output[6]=char((min / 10)+48);
 
    output[7]=char((min % 10)+48);
    ...
    {
    if (sec<10)
        output[9]=48; // "0" en ASCII
    else
        output[9]=char((sec / 10)+48);
    output[10]=char((sec % 10)+48);
    output[18]=0;
    }
    return (char*)&output;
}